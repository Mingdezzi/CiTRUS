# 파일 경로: tabs/easel/easel_controller.py (Restore Grid Logic + Keep Linear)
...
"""
    def __init__(self, view):
        self.view = view # EaselTabView 참조
        ...
        self.settings = {
            ...
            'grid_overlap': tk.IntVar(value=70), # <<< 그리드, 직선 공통 간격 옵션
        ...
        self.line_start_point = None # (x, y) 튜플 저장
        self.line_end_point = None   # (x, y) 튜플 저장
    ...
    def delete_layers(self, layers_to_delete: list[Layer]):
        if not layers_to_delete: messagebox.showinfo("알림", "삭제할 항목을 선택해주세요."); return
        ...
        if messagebox.askyesno("삭제 확인", f"{len(layers_to_delete)}개 항목 삭제?\n({layer_names})"):
            ...
            self.view.layer_list.populate_list(self.layers); self.update_status(f"{len(layers_to_delete)}개 삭제 완료.")
    ...
    def clear_all(self):
        if not messagebox.askyesno("확인", "모든 작업 내용을 초기화하시겠습니까?"): return
        ...
        self.delete_logo(confirm=False); self.settings['style_code'].set("");
        # [ ★★★★★ MODIFIED: reset_grid 호출 ★★★★★ ]
        self.reset_grid() # <<< 그리드 초기화 호출
        self.view.layer_list.populate_list(self.layers); self.update_status("모든 항목 초기화.")

    # --- 레이어 목록(List) 관련 로직 (변경 없음) ---
    ...
    def load_project(self):
        if not messagebox.askyesno("불러오기 확인", "현재 작업 내용 사라짐. 계속?"): return
    ...
    def remove_layer_background(self, layer: Layer):
        ...
        self.update_status(f"'{layer.get_display_name()}' 배경 제거 중..."); self.view.update_idletasks()
        ...
        else: print(f"DEBUG: BG remove failed for {layer.path}"); self.update_status("배경 제거 실패.")
    ...
    def apply_global_scale(self):
        ...
        [l.scale_var.set(scale_val) for l in selected if hasattr(l, 'scale_var')]
    ...
    def delete_logo(self, confirm=True):
        ...
        if confirm and not messagebox.askyesno("확인", "로고 삭제?"): return
        ...
        self.settings['logo_path'].set(""); self.update_logo_preview(); self.update_status("로고 삭제.") if confirm else None
    ...
    def on_logo_panel_drop(self, event):
        ...
        self.settings['logo_path'].set(os.path.normpath(first)); self._add_logo_to_canvas() if first else messagebox.showwarning("오류", ".png 파일 없음.")
    ...
    def update_logo_preview(self):
        ...
        if path and os.path.exists(path):
            try:
                ...
                traceback.print_exc()
                self.logo_preview_label.config(image="", text="오류") # Print traceback
    ...
    def apply_grid_layout(self):
        ...
        if work_w <= 1 or work_h <= 1: messagebox.showerror("오류", "캔버스 작업 영역 계산 오류."); return

        checked_cells = sorted([(r,c) for r, row in enumerate(self.view.grid_vars) for c, var in enumerate(row) if var.get()])
        num_images = len(images_to_layout); dests = []

        if not checked_cells: # 그리드 미선택 시 자동 그리드
             # 그리드 크기를 view에서 가져옴 (GRID_SIZE 사용)
        ...
        else: # 그리드 선택 시 '사이/주변' 배치 로직 (이전 버전 복원)
        ...
        optimal_scale = 10.0; low = 1.0; high = 500.0; iterations = 25 # low=1.0으로 수정
        print(f"DEBUG: Starting grid optimal scale search for {len(image_data)} images. Work area: {work_w:.1f}x{work_h:.1f}")
        for i in range(iterations):
            current_scale = (low + high) / 2; print(f"DEBUG: Grid Iter {i+1}, scale: {current_scale:.2f}")
            ...
            if not valid_sizes or len(valid_sizes) != len(image_data): # 모든 이미지 크기 유효해야 함
                 print(f"DEBUG: Invalid sizes found. Reducing scale."); high = current_scale; continue
            avg_w = sum(s[0] for s in valid_sizes) / len(valid_sizes); avg_h = sum(s[1] for s in valid_sizes) / len(valid_sizes); avg_w, avg_h = max(1, avg_w), max(1, avg_h);
            print(f"DEBUG: Avg grid size: {avg_w:.1f}x{avg_h:.1f}")
            placements = [{'x':(d['grid_c']-min_c)*avg_w*spacing, 'y':(d['grid_r']-min_r)*avg_h*spacing, 'w':s[0], 'h':s[1]} for d, s in zip(image_data, valid_sizes)] # valid_sizes 사용
            ...
            req_w, req_h = max_x - min_x, max_y - min_y; print(f"DEBUG: Grid Req area: {req_w:.1f}x{req_h:.1f}")
            if req_w < work_w and req_h < work_h: print(f"DEBUG: Grid Fits. Trying larger."); optimal_scale = current_scale; low = current_scale
            else: print(f"DEBUG: Grid Too large. Reducing."); high = current_scale
        print(f"DEBUG: Grid Optimal scale: {optimal_scale:.2f}")
        ...
        for l in self.layers: l.selected = (l in layers_to_scale)
        ...
        for d in image_data:
            ...
            if obj_info and obj_info.get('pil_for_display'):
                ...
                print(f"WARN: Could not get updated display info for layer {d['layer'].path} after grid scaling.")

        if not scaled_data: messagebox.showwarning("배치 오류", "스케일 적용 후 이미지 크기 정보를 얻지 못했습니다."); return

        avg_w = sum(s['w'] for s in scaled_data) / len(scaled_data) if scaled_data else 1; avg_h = sum(s['h'] for s in scaled_data) / len(scaled_data) if scaled_data else 1;
        ...
        print(f"DEBUG: Applying final grid placement to {len(placements)} images.")
        for p in placements:
            final_x = p['x'] + offset_x; final_y = p['y'] + offset_y; final_x = max(p['w']/2, min(final_x, work_w - p['w']/2));
    ...
    # --- [ ★★★★★ RENAMED & MODIFIED: 직선 배치 로직 ★★★★★ ] ---
    def apply_linear_layout(self, start_point, end_point):
        """주어진 시작점과 끝점을 따라 이미지들을 배치합니다."""
        print(f"DEBUG: apply_linear_layout called with Start: {start_point}, End: {end_point}")
        ...
        # 1. 작업 영역 계산 (배치 로직은 100% 줌 기준 캔버스 좌표 사용)
        ...
        line_angle_rad = math.atan2(dy, dx) # 라디안 각도
        line_angle_deg = math.degrees(line_angle_rad) # 도 각도 (회전 옵션용)

        print(f"DEBUG: Line Length: {line_length:.1f}, Angle: {line_angle_deg:.1f} degrees")
        
        # 3. 최적 스케일 탐색 (옵션 - 여기서는 일단 생략하고 일괄 크기 적용)
        # TODO: 필요 시 apply_grid_layout의 이진 탐색 로직 적용
        optimal_scale = self.settings['global_scale'].get() # 현재 설정된 일괄 크기 사용

        # 4. 이미지들에 스케일 적용 (일괄 크기 사용 시)
        # 이미 스케일이 적용된 상태라고 가정하거나, 필요시 여기서 apply_global_scale 호출
        ...
        for i, layer in enumerate(images_to_layout):
            ...
            # 이미지 순서(i)에 따라 직선 상의 위치 비율(t) 계산
            t = 0.5 # 기본값 (이미지 1개)
            if num_images > 1:
                t = i / (num_images - 1)
            ...
            # TODO: 이미지 회전 옵션
            # if rotate_option_enabled:
            #    layer.angle = line_angle_deg
            #    self.view.canvas_controller.update_object_display(layer, self.get_zoom()) # 회전 적용
            ...
            print(f"DEBUG: Placed {os.path.basename(layer.path)} at t={t:.2f} -> ({final_x:.1f}, {final_y:.1f})")
    ...
    def _get_temp_display_size(self, layer: Layer, scale: float, actual_zoom: float) -> tuple[int, int] | None:
        """주어진 스케일과 줌 레벨에서 레이어의 예상 표시 크기(W, H)를 계산 (PIL 객체 생성 안 함)"""
        # (이 함수는 그리드/각도 배치 시 최적 스케일 계산에 활용됨)
        if isinstance(layer, ImageLayer):
            ...
            if layer.angle != 0:
             ...
             if layer.angle != 0: return int(size * 1.414), int(size * 1.414)
             else: return int(size), int(size)
        return None

    # [ ★★★★★ RESTORED: 그리드 초기화 함수 ★★★★★ ]
    def reset_grid(self):
        [var.set(False) for row in self.view.grid_vars for var in row]
        self.update_status("그리드 선택 초기화.")

    # [ ★★★★★ NEW: 직선 배치 모드 시작 함수 ★★★★★ ]
    def start_linear_placement_mode(self):
        """직선 배치 모드를 시작하고 사용자 입력을 기다립니다."""
        if self.is_line_placement_mode: # 이미 모드 진행 중이면 취소
        ...
        self.view.canvas.config(cursor="crosshair") # 커서 변경
        # 버튼 텍스트 변경 (취소 기능)
        ...
        print("DEBUG: Entered Linear Placement Mode")
    ...
    def pick_color_from_canvas(self, event):
        ...
        if obj_info:
            ...
            if layer and hasattr(layer, 'color'):
                color = layer.color
                print(f"DBG: Color from layer attr: {color}")
                ...
                try:
                    ...
                    if 0 <= ix < w and 0 <= iy < h:
                        ...
                        print(f"DBG: Color from pixel: {color}" if color else f"DBG: Transparent pixel alpha={p[3]}")
                    else:
                        print(f"DBG: Coords outside image: ({ix},{iy}) vs ({w},{h})")
                except Exception as e:
                    self.update_status(f"⚠️ 추출 오류: {e}")
                    print(f"ERR: Pixel extract fail: {e}")
                    traceback.print_exc() # Print full traceback
            else:
                print(f"DBG: No color attr or PIL: {path}")
    ...
    def _exit_color_pick_mode(self, msg=""): self.is_color_picking_mode = False; self.view.canvas.config(cursor=""); self.update_status(msg or "색상 추출 모드 해제.")